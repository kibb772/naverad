...
export async function POST(req: NextRequest) {
  try {
    ...
    if (pendingDates.length === 0) {
      return NextResponse.json({ message: '이미 90일치 데이터가 수집되어 있습니다.', ready: true });
    }

    // 캠페인 → 광고그룹 → 키워드 목록 먼저 가져오기
    ...
    return NextResponse.json({
      message: `초기 데이터 수집 완료 (${totalSynced}일치, 키워드 ${allKeywords.length}개)`,
      ready: true,
      syncedDays: totalSynced,
    });
  } catch (error) {
    console.error('Initial sync error:', error);
    return NextResponse.json({ error: '초기 데이터 수집 중 오류가 발생했습니다.' }, { status: 500 });
  }
}
